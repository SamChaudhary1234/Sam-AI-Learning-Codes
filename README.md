...
- **Specialist Agent** — fine-tuned Llama running on Modal (serverless GPU)
- **Frontier Agent** — RAG-based pricer using GPT-5.1 + Chroma + sentence-transformers
- **Neural Network Agent** — wraps the 289M-param residual network from week 6
- **Ensemble Agent** — combines all three (80% / 10% / 10% weighted)
- **Scanner Agent** — parses RSS feeds, picks 5 best deals via structured outputs
- **Messaging Agent** — Pushover push notifications, message text crafted by Claude
- **Planning Agent** — orchestrates the full workflow; autonomous variant uses tool-calling

UI built with Gradio. Final benchmark: Ensemble achieves $29.90 mean error.
...
├── week4/   # Code generation (Python → C++ / Rust)
...
```bash
git clone https://github.com/SamChaudhary1234/Sam-AI-Learning-Codes.git
cd Sam-AI-Learning-Codes
uv sync
```

Add your API keys to `.env` (OpenAI, Anthropic, Google, Groq, HuggingFace, Pushover as needed).

For weeks 7/8 you'll need a GPU — Colab works. Drop the trained `deep_neural_network.pth` weights into `week6/` and `week8/` before running the ensemble code.

## Contact

Sam Chaudhary · [LinkedIn](https://linkedin.com/in/sam-chaudhary-75351a259) · samchaudharywork@gmail.com
